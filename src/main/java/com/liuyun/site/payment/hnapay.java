...
import com.liuyun.site.util.ByteArrayUtil;
import com.liuyun.site.util.CharsetTypeEnum;
import com.liuyun.site.util.HashAlgorithms;
import com.liuyun.site.util.RSAAlgorithms;
...
public class hnapay {
	...
	public static String genSignByRSA(String src, CharsetTypeEnum charsetType)
		...
		try {
			InputStream in = hnapay.class.getClassLoader().getResourceAsStream(
	...
	public static String genSignByMD5(String src, CharsetTypeEnum charsetType)
			throws Exception {
		if (!flag) {
	...
	public static boolean verifySignatureByRSA(String src, String dit,
		...
		try {
			...
			result = sign.verifySignature(hashSrc.getBytes(charsetType
					.getDescription()), dit, ByteArrayUtil
					.toByteArray((String) paramsMap.get("hnapay.gateway.pubkey")));
		} catch (Exception e) {
			System.err.println("验证签名出现异常：请检查输入参数");
			e.printStackTrace();
			throw new Exception(e);
