...
public class GoPay extends Pay {
	...
	private String getSignVal() {
		...
		if (com.rd.util.StringUtils.isBlank(getGood().getFeeAmt()))
			sb.append("feeAmt=[]");
		else {
		...
		sb.append("respCode=[]");
		sb.append("gopayServerTime=[").append(com.rd.util.StringUtils.isNull(this.servetime)).append("]");
	...
	public String submitGet() {
		...
		url.append("userType=").append(com.rd.util.StringUtils.isNull(getUserType())).append("&");
		url.append("bankCode=").append(com.rd.util.StringUtils.isNull(getBankCode())).append("&");
